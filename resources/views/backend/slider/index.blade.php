                    ...
                    <div class="card mb-3">
                        <div class="card-header">
                            <span class="pull-right">
                                <button id="modal_add" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal_add_slide"><i class="fa fa-plus" aria-hidden="true"></i> Add slider image</button>
                            </span>
                            ...
                            <h3><i class="fa fa-image"></i> All Slider ({{count($data)}} images)</h3>
                        </div>
                        <!-- end card-header -->
                        <div class="card-body">
                            <div class="alert alert-danger err_link">
    ...
    <script>
        $(document).ready(function() {
               ...
               $('#btn_add_cate').click(function(event) {
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if(data.status == 0) {
                                if(data.messages.code == 204) {
                                    $('.err_link').css('display', 'block').text(data.messages.message);
                                } else {
                                    if(data.messages.errors.link.length != 0) {
                                        // console.log(data.messages.errors.link[data.messages.errors.link.length-1])
                                        $('.err_link').css('display', 'block').text(data.messages.errors.link[data.messages.errors.link.length-1])
                                    }
                                }
                        ...
                        error: function(err) {
                            // console.log(err)
                            $('.err_link').css('display', 'block').text('Kích thước ảnh phải nhỏ hơn 2mb');
                            $('.close').trigger('click');
                        }
                    });
            ...
            $('body').on('click', '#modal_add', function() {
                $('#modal_add_slide').find('.modal-title').text('Add image slider');
                $('#modal_add_slide').find('#btn_add_cate').text('Add');
                $('#g_title').val('');
                $('#img').val('');
                $('.err_link').css('display', 'none').text('');
                $('#avatar').attr('src', "{{url('/uploads/new_seo-10-512.png')}}");
                addUpdateCateDB('add');
            });
            $('body').on('click', '.btn_edit', function() {
                var link = url+'/'+$(this).data('link');
                $('#modal_add_slide').find('.modal-title').text('Update image slider');
